...
publishDate: 2026-08-06T01:01:00+09:00
draft: false
tags:
  - "devops"
  - "github-actions"
  ...
  - "deployment"
  - "performance"
excerpt: "운영 중인 쇼핑몰의 배포 로그에서 QEMU 병목을 찾고, GitHub Actions build job 한 곳만 native ARM runner로 바꿔 검증한 기록입니다."
---
...
### 반복 실행의 중앙값도 같은 방향이었다

직전과 직후 한 번만 비교하면 캐시나 runner 상태에 따른 우연일 수 있습니다. 그래서 성공한 전환 전 5회와 전환 후 5회의 중앙값도 함께 비교했습니다.

![QEMU 교차 빌드와 ARM64 네이티브 빌드의 성공 실행 각 5회 중앙값 비교](_assets/how-native-arm-runner-cut-nextjs-build-time/build-time-comparison.jpg)

| 측정 구간        | QEMU 교차 빌드 | ARM64 네이티브 | 변화             |
| ---------------- | -------------: | -------------: | ---------------- |
| 전체 검증·배포   |          534초 |          376초 | 30% 단축, 1.42배 |
| `build-and-push` |          267초 |           98초 | 63% 단축, 2.72배 |
| Web 이미지 빌드  |          190초 |           33초 | 83% 단축, 5.76배 |
| API 이미지 빌드  |           11초 |            7초 | 36% 단축, 1.57배 |

가장 큰 차이는 Web 이미지 빌드였습니다. 중앙값이 190초에서 33초로 줄어 약 5.76배 빨라졌습니다. 반면 Docker 밖에서 실행한 API JAR 빌드는 47초에서 48초로 거의 변하지 않았습니다.

모든 작업이 함께 빨라진 것이 아니라 Docker 안에서 ARM64 프로그램을 실행하던 구간에서 차이가 집중됐습니다. 이는 GHCR이나 전반적인 runner 성능보다 Docker 빌드 내부의 ARM64 에뮬레이션이 주요 병목이었다는 판단을 뒷받침합니다.
...
이번 기록에는 두 종류의 비교가 섞여 있습니다.

- Docker 안의 동일한 `RUN npm run build` 구간은 전환 직전과 직후의 실제 운영 배포를 한 번씩 비교했습니다.
- workflow, Web·API 이미지 구간은 전환 전후 성공 실행을 각각 5회 모아 중앙값으로 비교했습니다.

반복 실행의 중앙값을 추가해 한 번의 결과만 볼 때보다 신뢰도를 높였지만, 완전히 통제된 벤치마크는 아닙니다.

- 실행마다 commit이 서로 다를 수 있습니다.
...
따라서 “모든 Next.js ARM 빌드가 13.7배 빨라진다”고 일반화할 수는 없습니다. 다만 Dockerfile의 동일한 `RUN npm run build` 단계가 188.3초에서 13.7초로 줄었고, 5회 중앙값에서도 Web 이미지 빌드가 190초에서 33초로 줄었습니다. workflow에서 바꾼 핵심 변수는 build job의 아키텍처와 QEMU 제거였습니다. 이번 병목을 설명하기에는 충분히 일관된 차이였습니다.
