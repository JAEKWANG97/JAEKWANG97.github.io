...
**사이트 운영자가 자기 서비스 전용 MCP 서버를 따로 구현해야 하는 것은 아닙니다.** 페이지에 기존 기능을 WebMCP 도구로 등록할 수 있습니다. 다만 페이지의 도구 등록과 에이전트의 브라우저 연결은 별개입니다. 등록만으로 모든 에이전트에 자동 연결되지는 않습니다.[4]

외부 에이전트를 연결하는 한 가지 수단이 Chrome 팀의 **Chrome DevTools MCP**입니다. 에이전트와 Chrome 사이에서 페이지 도구의 조회·호출을 중개하는 MCP 서버입니다. 뒤의 POC에서는 이 연결을 사용했으며, 먼저 페이지 안에서 검색이 실행되는 원리를 살펴보겠습니다.[7][8]
...
**Chrome DevTools MCP를 Codex CLI에 연결해 WebMCP POC를 진행했습니다.** 이 블로그의 [글 검색 페이지](/search/)에 등록한 `search_posts`를 에이전트가 발견하고 호출했으며, 검색 결과 반환과 화면 갱신까지 확인했습니다. Chrome DevTools MCP는 에이전트와 Chrome을 연결하는 수단이고, WebMCP는 이번에 검증한 페이지 도구 인터페이스입니다.

실행 환경은 2026년 9월 8일 기준 macOS, Chrome 152.0.7977.76, Chrome DevTools MCP 1.8.0입니다. Herdr에서 새 Codex CLI 세션을 열었지만, Herdr는 터미널 탭을 관리할 뿐 WebMCP를 중개하는 구성요소는 아닙니다.
...
공식 안내도 출처를 구분해서 볼 필요가 있습니다. Chrome의 WebMCP 개요 문서는 Model Context Tool Inspector 확장으로 도구를 테스트하는 방법을 소개합니다. 한편 Chrome DevTools MCP 공식 저장소는 WebMCP 조회·실행 도구와 Codex CLI 연결 방법을 제공합니다. 이번 POC는 후자의 연결 경로를 사용한 사례이며, WebMCP를 쓰는 유일한 방법은 아닙니다.[1][7][8]

설치 명령과 요청 문구는 글 끝의 [POC 재현 안내](#poc-재현-안내)에 모았습니다.
...
사용자에게는 자연어 요청이 화면의 변화로 이어지는 경험이고, 웹서비스에는 기존 기능을 에이전트도 이용할 수 있게 여는 인터페이스입니다. 이번 POC에서 Chrome DevTools MCP는 그 연결을 구현한 수단이었습니다. 글의 핵심은 특정 연결 도구가 아니라, **사이트가 공개한 기능을 에이전트가 발견하고 직접 호출한다는 변화**입니다.

## POC 재현 안내

직접 따라 해보고 싶은 분을 위한 연결 설정입니다. 앞의 개념 설명을 이해하는 데 설치가 필요한 것은 아닙니다.
...
“이 블로그에서 WebMCP 관련 글을 찾아줘”라는 요청을 검증하기 위해, 에이전트에는 다음처럼 대상 URL과 호출 방식을 명시했습니다.
...
도구가 목록에 보이지 않으면 먼저 연결과 등록 상태를 확인해야 합니다. 검색창을 클릭해 같은 결과를 얻었다고 해서 WebMCP 호출을 검증한 것은 아닙니다.
