...
publishDate: 2026-03-07T00:00:00+09:00
draft: false
tags:
  - "open-source"
  - "line"
  ...
  - "contribution"
mermaid: true
excerpt: "--- 관련 이슈(6422)의 설명은 비교적 짧았습니다. 정리하면, 헤더 이름이 프레임워크 내부에 고정되어 있어 사용자 지정 헤더를 사용할 수 없다는 점이 문제였습니다. 이 상태에서 커스텀 헤더를 쓰려면 Armeria의 Athenz 모듈을 온전히 활용하기 어렵고, 클라이언트/서"
---
...
![2026-03-07-21-27-24](_assets/line-armeria-athenz-token-header-contribution/2026-03-07-21-27-24.png)
...
![2026-03-07-21-57-34](_assets/line-armeria-athenz-token-header-contribution/2026-03-07-21-57-34.png)
...
메인테이너 분들의 리뷰를 반영하면서 실제로 코드와 문서가 함께 바뀌었습니다. 기존 API가 Deprecated 되었을 때는 Javadoc에도 변경 사항과 대체 경로를 명확히 반영해야 했고, 사용자 편의를 위해 팩토리 메서드도 필요했습니다. 또한 `header()`처럼 의미가 넓은 이름은 `tokenHeader()`로 바꿔 의도를 분명히 했고, Deprecated API는 새 API로 자연스럽게 포워딩되도록 정리해 마이그레이션 경로를 만들었습니다. 이런 과정을 거쳐 PR이 머지됐고,

_“Thanks @JAEKWANG97”_ 코멘트를 볼 수 있었습니다.

![2026-03-07-21-29-35](_assets/line-armeria-athenz-token-header-contribution/2026-03-07-21-29-35.png)

---

### 마무리: 배운 점

이번 기여를 통해 오픈소스 생태계에서 **하위 호환성을 해치지 않으면서 확장 포인트를 설계하는 방법**을 실제 프로덕션 레벨에서 적용해 구현할 수 있었습니다. 단순한 기능 구현을 넘어, API 네이밍과 테스트 기반 구성, 예외 케이스 방어까지 종합적으로 고민하는 소중한 경험이었습니다.
